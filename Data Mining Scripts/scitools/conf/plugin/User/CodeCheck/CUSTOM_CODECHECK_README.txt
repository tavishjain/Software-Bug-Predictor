...
3. Now modify the check subroutine to include the regular expression check and to signal a codecheck violation reporting the problem. Add this code at the end of the subroutine:</>

  if ($file->name =~ /^[^a-zA-Z]/){
    $check->violation(0,$file,-1,-1,"File name does not begin with a letter");  
  }
4. The last step is to verify that the perl syntax is correct. The easist way to do this is to open a command line and run the perl application that ships with Understand: uperl -c mysample.upl.
